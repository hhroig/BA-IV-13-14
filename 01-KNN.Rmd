...
Seguiremos con los datos `iris`. El paso inicial: análisis dsecriptivo y visualización de los datos podríamos obviarlo... pero a modo didáctico reproducimos el mismo análisis, esta vez usando la función `featurePlot` de `caret`.
...
Aunque esto no debe usarse como método de selección de variables, sí motiva el estudio del problema al disminiuir la dimensión $p = 4$. Por ejemplo, veamos la precisión del modelo al dejar solo `Petal.Length` y `Petal.Width`.
...
Abordamos ahora el problema de regresión con KNN, o sea, la respuesta es cuantitativa-continua. Seguimos usando el paquete `caret` que tiene implementado el algoritmo y ofrece vacilidades para el preprocesado de los datos y la validación del modelo. 
